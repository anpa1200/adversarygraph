# AdversaryGraph v7 Release Readiness

This is the current acceptance baseline for the controlled self-hosted
`v7.0.0` release candidate. “Production ready” means every applicable item below has an
owner and evidence for the exact deployed tag and images; it does not mean the
default stack is a managed or multi-tenant SaaS. Historical release evidence
does not validate the v7 source. Accept v7 only when the exact merge commit,
protected tag workflow, eight-image digest manifest, and deployment evidence
all agree.
...
- fresh strict-scan container builds that pull current base-image metadata and
  bypass the Docker layer cache before scanning all eight custom image
  families.
...
the critical tests, deployment renders, secret scan, eight custom-image scans,
and three pinned stack-image scans before the v7 tag can publish packages or
...
v7 hardening: evidence for older tags must be evaluated against the workflow
and commit stored at each tag.

### Prepared-source validation record

The v7 source candidate was validated locally on 12 August 2026 before the
release commit. The reproducible results were:

- 757 backend tests passed and 10 were skipped, with 68.01% measured coverage
  against the enforced 60% floor;
- 53 Playwright browser tests passed and four screenshot-only cases were
  skipped;
- all eight scanner MCP tests passed;
- the generated contract inventory covered 323 OpenAPI operations, 28 backend
  API modules, 288 frontend API call sites, and all 31 governed workspaces;
- frontend and Anomaly Detection Atlas documentation production builds passed,
  including the vendored image-parser boundary test;
- Ruff, Bandit, pip-audit, npm audit policy, Gitleaks, Compose rendering,
  production-environment policy, Helm lint/render, and strict Trivy checks
  passed for every custom release image;
- Redis and Nginx pinned stack images passed the strict scan; the pinned
  BusyBox image exposed no supported package inventory or actionable finding.

This record describes the prepared source and local build environment. It does
not replace merge CI, the protected tag workflow, registry digest verification,
or a deployment-specific authenticated application self-test.

### Fresh-container and v7 publication policy

The checked-out v7 source configures strict local container scans
...
For the v7.0.0 version tag, the current workflow builds each of the eight release
...
tag completes successfully; it does not retroactively describe an older tag.
...
PostgreSQL, backend, frontend, MalwareGraph, and scanner MCP. Its PostgreSQL and Redis
evaluation defaults are digest-pinned, while backend, frontend, and
MalwareGraph and scanner MCP use the human-readable `7.0.0` candidate tag with
empty digest fields. Those tags support evaluation only; production also replaces
PostgreSQL and supplies revision-matched images with reviewed manifest digests
for all enabled release components.
...
| Images | Fresh no-cache image scans pass; fixable high/critical findings are absent; published registry digests are recorded and used where the deployment supports them | Tag-workflow run, eleven stack scan results, registry digest record, rendered deployment |
...
The following remain outside the v7.0.0 controlled self-hosted production
claim:
