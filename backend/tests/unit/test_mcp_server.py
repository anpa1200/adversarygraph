...
@pytest.mark.asyncio
async def test_remote_assistant_requires_operator_gate_and_per_call_ack(monkeypatch, local_api):
    async def should_not_run(*_args, **_kwargs):
        pytest.fail("API request must not run before remote-processing policy passes")
    ...
    with pytest.raises(mcp_server.MCPInputError, match="must be false"):
        await mcp_server.ask_intelligence(
            "Summarize T1059.001",
            provider="local",
            cloud_processing_acknowledged=True,
...
@pytest.mark.asyncio
async def test_remote_assistant_passes_explicit_provider_and_ack(monkeypatch, local_api):
    captured = {}
    monkeypatch.setattr(mcp_server.settings, "mcp_remote_ai_enabled", True)
    ...
    monkeypatch.setattr(mcp_server, "_request_json", fake_request)
    result = await mcp_server.ask_intelligence(
        "Summarize T1059.001",
        provider="claude",
        cloud_processing_acknowledged=True,
    )
    ...
    assert result["answer"] == "PowerShell evidence [S1]"


def test_assistance_transport_timeout_exceeds_governed_provider_ceiling():
