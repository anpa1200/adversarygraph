...
from app.models.pipeline import AuditEvent, CollectionRun, CollectionSource
                    ...
                    try:
                        ...
                        for item in entries:
                            existing = await db.execute(
                                select(ReportIntake).where(ReportIntake.url == item["url"], ReportIntake.title == item["title"])
                            )
                            ...
                            run.items_created += 1
                            # Candidate indicators remain on ReportIntake. The
                            # Report Review Gate owns every global Observable
                            # or retrohunt side effect derived from a report.
                        run.status = "complete"
